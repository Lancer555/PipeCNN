...
//Natan :: Define custom data types. Taken from $INTELFPGAOCLSDKROOT/include/kernel_headers/ihc_apint.h
...
channel channel_scal_mac        bypass_ch_mac                   __attribute__((depth(CHN_DEPTH)));
channel MACTYPE                 thresholds_ch                   __attribute__((depth(CHN_DEPTH)));

// For Batchobrm
#ifdef BATCHNORM
channel channel_scal            gamma_ch                        __attribute__((depth(8)));
channel channel_scal            beta_ch                         __attribute__((depth(8)));
channel channel_scal_mac        mean_ch                         __attribute__((depth(CHN_DEPTH)));
channel channel_scal_mac        std_ch                          __attribute__((depth(CHN_DEPTH)));
#endif
...
MACTYPE mac(lane_data input, lane_data weights)
{
    ...
    for(int i=0; i<VEC_SIZE/4; i++){
        //printf("%d x %d + %d x %d + %d x %d + %d x %d",input.data[i*4], weights.data[i*4], input.data[i*4+1], weights.data[i*4+1], input.data[i*4+2], weights.data[i*4+2], input.data[i*4+3], weights.data[i*4+3]);
        ...
        //printf(" = %d  . Actual = %d \n",input.data[i*4]* weights.data[i*4]+ input.data[i*4+1]*weights.data[i*4+1]+ input.data[i*4+2]*weights.data[i*4+2] +  input.data[i*4+3] * weights.data[i*4+3], output);
...
/*
DPTYPE pool_avg(DPTYPE a_in, DPTYPE b_in)
{
    return (a_in + b_in) >> 1; //Average of two numbers

}
*/

DPTYPE threshold(MACTYPE act_in, DPTYPE layer_num, MACTYPE *thresholds)
{




    DPTYPE activation = THRESHOLDS_NUM;
    #pragma unroll
    for(DPTYPE i=0; i < THRESHOLDS_NUM; i++){
        if (act_in < thresholds[i]) {
            activation = i;
            break;
        }
    }   
...
void memRead(
            ...
            __global int                *restrict thresholds

            //For batchnorm
            /*
            __global channel_scal_mac   *restrict mean,
            __global channel_scal_mac   *restrict std,
            __global channel_scal       *restrict gamma,
            __global channel_scal       *restrict beta
            */
...

{
    ...
    channel_scal  thresholds_ch_in;

    //For batchnorm
    channel_scal_mac   mean_ch_in;
    channel_scal_mac   std_ch_in;
    channel_scal       gamma_ch_in;
    channel_scal       bete_ch_in;
    ...
    //#pragma unroll
    //for (char t =0; t < THRESHOLDS_NUM; t++){ // Natan 
    //    write_channel_intel(thresholds_ch[t], thresholds[t]);
    //}

    for (char t =0; t < THRESHOLDS_NUM; t++){ // Natan
        write_channel_intel(thresholds_ch, thresholds[t]);
    }
...
#ifdef BATCHNORM
                                    mean_ch_in = mean[out_idx_z];
                                    std_ch_in = std[out_idx_z];
                                    gamma_ch_in = gamma[out_idx_z];
                                    bete_ch_in = beta[out_idx_z];
                                    write_channel_intel(mean_ch, mean_ch_in);
                                    write_channel_intel(std_ch, std_ch_in);
                                    write_channel_intel(gamma_ch, gamma_ch_in);
                                    write_channel_intel(beta_ch, bete_ch_in);
#endif
...
void coreConv(
            ...
            char  frac_din,
            char  frac_dout,
            char  layer_num //Relevant when using thresholds
            )
{
    ...
    MACTYPE conv_final[LANE_NUM]; 

    MACTYPE thresholds[THRESHOLDS_NUM]; 
    //DPTYPE  conv_final[LANE_NUM];

    channel_scal conv_th_ch_in;
    channel_scal_mac conv_th_ch_in_mac;

#ifdef BATCHNORM
    channel_scal_mac    std_ch_out;
    channel_scal_mac    mean_ch_out;
    channel_scal        gamma_ch_out;
    channel_scal        beta_ch_out;

    DPTYPE              gamma[LANE_NUM];
    DPTYPE              beta[LANE_NUM];
    MACTYPE             std[LANE_NUM];
    MACTYPE             mean[LANE_NUM];

#endif

    //#pragma unroll
    //for(unsigned char t=0; t<THRESHOLDS_NUM; t++){
    //    thresholds[t] = read_channel_intel(thresholds_ch[t]);
    //}

    for(unsigned char t=0; t<THRESHOLDS_NUM; t++){
        thresholds[t] = read_channel_intel(thresholds_ch);
        //printf(" %d ",thresholds[t]);
    }
...
#ifdef BATCHNORM
        std_ch_out      = read_channel_intel(std_ch);
        mean_ch_out     = read_channel_intel(mean_ch);
        gamma_ch_out    = read_channel_intel(gamma_ch);
        beta_ch_out     = read_channel_intel(beta_ch);
#endif
...
#ifdef BATCHNORM
            gamma[ll]  = std_ch_out.lane[ll];    
            beta[ll]   = mean_ch_out.lane[ll];   
            std[ll]    = gamma_ch_out.lane[ll];  
            mean[ll]   = beta_ch_out.lane[ll];   
#endif 
...
*/
            ...
            conv_final[ll] = conv_out[ll] + bias[ll]; //Natan :: Adding Bias term

#ifdef BATCHNORM
            conv_final[ll] = ((conv_out[ll] - beta[ll])/(std[ll])  * gamma[ll]) + beta[ll]; //Natan :: Adding Bias term
#endif
            ...
            if((contol&0x01)==0x01){
                if((conv_final[ll]&MASKSIGN)==MASKSIGN)
                    ...
                    conv_ch_in[ll] = conv_final[ll];
                    //conv_ch_in.lane[ll] = conv_final[ll];
            }
            else
                conv_ch_in[ll] = conv_final[ll];
                //conv_ch_in.lane[ll] = conv_final[ll];
            if (layer_num != LAYER_NUM-1)
                conv_th_ch_in.lane[ll] = threshold(conv_ch_in[ll], layer_num, thresholds); //Natan :: Added threshold function
            else
                conv_th_ch_in_mac.lane[ll] = conv_ch_in[ll]; //Natan :: Added threshold function

            //conv_th_ch_in.lane[ll] = threshold(conv_ch_in.lane[ll], layer_num); //Natan :: Added threshold function
        ...
        if((contol&0x02)==0x02)
            //by-pass pooling
            if (layer_num != LAYER_NUM-1) 
                write_channel_intel(bypass_ch, conv_th_ch_in);
            else
                write_channel_intel(bypass_ch_mac, conv_th_ch_in_mac);
            //write_channel_intel(bypass_ch, conv_ch_in);
        else // to pooling kernel
            write_channel_intel(conv_ch, conv_th_ch_in);
...
void memWrite(
                ...
                //Natan :: The last layer doesnt preform thresholding, therefore it should transmit the whole MAC result, which is int
                __global int *restrict last_fc, //Natan Added
                uchar  layer_num
                ...
                )
{
    ...
    channel_scal   output;
    channel_scal_mac   output_mac;
    __local DPTYPE buffer[LANE_NUM];
    __local MACTYPE buffer_mac[LANE_NUM];
    ...
    if(local_z==0){
        if((bypass&0x01)==0x01)
            if (layer_num != LAYER_NUM-1)
                ...
                output_mac = read_channel_intel(bypass_ch_mac);
        ...
        for(uchar ll=0; ll<LANE_NUM; ll++){
            ...
            buffer_mac[ll]=output_mac.lane[ll];
        }
    }
    ...
    if((global_z-padd_offset)<out_dim3 && (global_z>=padd_offset)){
        if (layer_num != LAYER_NUM-1)
            ...
            last_fc[index_z_group*out_dim1x2xbatch*VEC_SIZE + (global_y+batch_indx_dim2*out_dim2)*out_dim1xbatch*VEC_SIZE + (global_x+batch_indx_dim1*out_dim1)*VEC_SIZE + index_z_item] = buffer_mac[local_z];
