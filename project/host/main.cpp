...
#ifdef CIFAR10
typedef  int  OUTDTYPE;
#else
typedef  signed char  OUTDTYPE;
#endif
...
char  picture_file_path_head[100]  = "/mnt/ssd/ImagNet_val/ILSVRC2012_val_";

#else

#define PICTURE_NUM 10000
#define MAX_PIC_NUM 10000
const char *LabelPath              = "/home/vista_fpga/Natan/quant_net_fpga_uniq/Data/cifar-10-batches-py/val.txt";
char  picture_file_path_head[100]  = "/home/vista_fpga/Natan/quant_net_fpga_uniq/Data/cifar-10-batches-py/test/CIFA10_val_"; // TBD :: Need to change to CIFAR10_val_ 
#endif
...
#define WEIGHTS_FILE_SIZE 14025866
#define LAYER_NUM         9
#define CONV_NUM          6
#define THRESHOLDS_NUM    15
#define THRESOLDS_FILE_SIZE (15*(LAYER_NUM-1))  

const char *weight_file_path = "/home/vista_fpga/Natan/quant_net_fpga_uniq/saved_parameters/quant_weights2018-06-11_02-50-01.dat";
const char *thresholds_file_path = "/home/vista_fpga/Natan/quant_net_fpga_uniq/saved_parameters/thresholds2018-06-11_02-50-01.dat";
...
MACTYPE *thresholds_conv[MAX_LAYER_NUM];
...
// DTYPE *golden_ref;
OUTDTYPE *output;
OUTDTYPE *output_one_item;
OUTDTYPE *output_reorder;
OUTDTYPE *golden_ref;
...
void getAccuracy(OUTDTYPE *output_reorder,int num);
...
void reorderThresholds(MACTYPE *thresholds, MACTYPE *thresholds_out, int layer);
...
void reorderOutput(OUTDTYPE *output, OUTDTYPE *output_reorder, unsigned dim1, unsigned dim2, unsigned dim3);
void extractOutput(OUTDTYPE *output, OUTDTYPE *output_one_item, unsigned item_num, unsigned batch_size, unsigned dim1, unsigned dim2, unsigned dim3);
void softmax(OUTDTYPE *output_reorder , OUTDTYPE *output);
int getProb(OUTDTYPE *output);
void cleanup();
...
int main(int argc, char** argv)
{
   ...
   if (getcwd(cwd, sizeof(cwd)) != NULL)
    ...
    thresholds_buf.reset(num_devices*LAYER_NUM);
    ...
    fc_2_buf_mac_type.reset(num_devices);
    ...
    for(unsigned i = 0; i < num_devices; ++i) {
        ...
        for(unsigned j = 0; j < LAYER_NUM; ++j){
            ...
            // Thresholds buffers for each layer
            #ifdef USE_THRESHOLDS
            thresholds_buf[i*LAYER_NUM+j] = clCreateBuffer(context, CL_MEM_READ_ONLY, THRESHOLDS_NUM * sizeof(int), NULL, &status);
            checkError(status, "Failed to create buffer for bias in layer");
            #endif
            ...
            status = clEnqueueWriteBuffer(que_memRd[i], thresholds_buf[i*LAYER_NUM+j], CL_TRUE, 0, THRESHOLDS_NUM * sizeof(int), thresholds_conv[j], 0, NULL, NULL);
            checkError(status, "Failed to transfer bias");
        ...
        checkError(status, "Failed to create buffer for data in fc layer");
        fc_2_buf_mac_type[i] = clCreateBuffer(context,  CL_MEM_READ_WRITE, FC_BUF_SIZE * sizeof(OUTDTYPE), NULL, &status);
        checkError(status, "Failed to create buffer for data in fc layer");
    ...
    for(unsigned i = 0; i < num_devices; ++i) {
        ...
        for(unsigned char j = 0; j < LAYER_NUM; ++j){
            ...
            status = clSetKernelArg(knl_memRd[i], argi++, sizeof(cl_mem), &thresholds_buf[i*LAYER_NUM+j]);
            checkError(status, "Failed to set argument %d kernel memRd", argi - 1);
            ...
            status = clSetKernelArg(knl_conv[i], argi++, sizeof(cl_char), &j); //Natan :: The number of layer is needed for selecting the thresholds
            checkError(status, "Failed to set argument %d of kernel conv", argi - 1);
            ...
            status = clSetKernelArg(knl_memWr[i], argi++, sizeof(cl_mem), &fc_2_buf_mac_type[i]); // Natan :: Final result MAC
            checkError(status, "Failed to set argument %d of kernel memWr", argi - 1);

            status = clSetKernelArg(knl_memWr[i], argi++, sizeof(cl_uchar), &j);                  //Natan added layer number
            checkError(status, "Failed to set argument %d of kernel memWr", argi - 1);
...
void readDataBack()
{
    ...
    else if(layer_config[LAYER_NUM-1][memwr_dst] == 4){
        printf("\nCopyed all batched results from fc_2 fc_2_buf_mac_type.\n");
        status = clEnqueueReadBuffer(que_memWr[0], fc_2_buf_mac_type[0], CL_FALSE,          // read from device0
            0, sizeof(OUTDTYPE) * read_buf_size, (void *)output, 0, NULL, &finish_event[0]);
        checkError(status, "Failed to set transfer output data");
    }
...
void verifyResult(int num)
{
    ...
    if(max_label == label[num-1]){
        ...
        printf("True: True_label = %d Inferred_label = %d\n\n", label[num-1], max_label);

    } else {
        putText(img,"False",Point(20, 80), CV_FONT_HERSHEY_SIMPLEX, 0.8, Scalar(0, 0, 255), 2, 8);
        printf("False: True_label = %d Inferred_label = %d\n\n", label[num-1], max_label);
        // printf("False: True_label = %d Inferred_label = %d\n\n", label[num], max_label); // label[num] should be label[num-1] since num starts from 1
    }
    imshow( "PipeCNN", img);
...
int prepare()
{
    ...
    for(unsigned ll=0; ll<LAYER_NUM; ll++){
        ...
        if(ll==0){ // check parameters for layer-1
            ...
            if((layer_config[ll][weight_n]%VEC_SIZE)!=0){
                printf("\nError: incorrect setting of parameter VEC_SIZE in Layer %d! weigh_n =%d VEC_SIZE = %d !!\n", ll, layer_config[ll][weight_n], VEC_SIZE);
                return 1;
            }
        ...
        if((layer_config_original[ll][conv_x]!=(layer_config_original[ll][data_w]-layer_config_original[ll][weight_w]+2*layer_config_original[ll][conv_padding])/layer_config_original[ll][conv_stride]+1)
            ...
            printf("cond 1 %d cond 2 %d cond 3 %d ",(layer_config_original[ll][conv_x]!=(layer_config_original[ll][data_w]-layer_config_original[ll][weight_w]+2*layer_config_original[ll][conv_padding])/layer_config_original[ll][conv_stride]+1),
                                                   (layer_config_original[ll][conv_y]!=(layer_config_original[ll][data_h]-layer_config_original[ll][weight_h]+2*layer_config_original[ll][conv_padding])/layer_config_original[ll][conv_stride]+1),
                                                   (layer_config_original[ll][conv_z]!=layer_config_original[ll][weight_m])
                 );
    ...
    #ifdef USE_THRESHOLDS
    thresholds   = (MACTYPE *)alignedMalloc(sizeof(MACTYPE)*THRESOLDS_FILE_SIZE, DMA_ALIGNMENT);
    #endif
    // input data buffers
    ...
    output          = (OUTDTYPE *)alignedMalloc(sizeof(OUTDTYPE)*output_size, DMA_ALIGNMENT); // vectorized results
    output_one_item = (OUTDTYPE *)alignedMalloc(sizeof(OUTDTYPE)*godref_size, DMA_ALIGNMENT); // one item extracted from batch results
    golden_ref      = (OUTDTYPE *)alignedMalloc(sizeof(OUTDTYPE)*godref_size, DMA_ALIGNMENT);
    output_reorder  = (OUTDTYPE *)alignedMalloc(sizeof(OUTDTYPE)*godref_size, DMA_ALIGNMENT); // reordered results for verifying
    #ifdef USE_THRESHOLDS
    if(weights == NULL || image == NULL || golden_ref == NULL || data_init == NULL || output == NULL || output_one_item == NULL || output_reorder == NULL || thresholds == NULL)
    #else
    ...
    for(int j=0; j<LAYER_NUM; j++){
        ...
        #ifdef USE_THRESHOLDS
        thresholds_conv[j]      = (MACTYPE *)alignedMalloc(sizeof(MACTYPE)*THRESOLDS_FILE_SIZE, DMA_ALIGNMENT);
        #endif
        ...
        memset(thresholds_conv[j], 0, sizeof(int)*THRESHOLDS_NUM);// reset all value (include padding value) to zero
    ...
    // Thresholds
    #ifdef USE_THRESHOLDS
    bin_file_r.open(thresholds_file_path, ios::in | ios::binary);
    ...
    if(bin_file_r.is_open())
    {
        ...
        bin_file_r.read((char *) thresholds, sizeof(DTYPE)*WEIGHTS_FILE_SIZE);  // Nessecery to read 32 bit integer
        printf("\n%d total thresholds read \n", file_size/(sizeof(MACTYPE)));
        if(THRESOLDS_FILE_SIZE!=(file_size/(sizeof(MACTYPE))))
            printf("Warning: thresholds file size does not match user configuration !!! expected %d  got %d \n");
        bin_file_r.close();
    }
    else
        printf("Thresholds file does not exits !!!\n");

    // for(unsigned m = 0; m<8; m++){
    //     for(unsigned n = 0; n<15; n++){
    //         printf("%d ", thresholds[m*15 +n]);
    //     }
    //     printf("\n");
    // }
    #endif
    ...
    if(bin_file_r.is_open())
    {
        ...
        printf("%d total output reference read \n\n", file_size/((int)sizeof(OUTDTYPE)));
        if(godref_size!=(file_size/(sizeof(OUTDTYPE))))
    ...
    #ifdef USE_THRESHOLDS
    reorderThresholds(thresholds,thresholds_conv[0], 0);
    #endif
    // Other layers
    for(unsigned j=1; j<LAYER_NUM; j++){
        ...
        #ifdef USE_THRESHOLDS
        reorderThresholds(thresholds,thresholds_conv[j], j);
        #endif
    }
...
void reorderWeights(DTYPE *weights, DTYPE *weight_buf, unsigned dim1, unsigned dim2, unsigned dim3, unsigned dim4, unsigned dim3_original, unsigned dim4_original, unsigned offset, unsigned padding_offset, unsigned vecSize, unsigned laneNum){
    ...
    // printf("dim1 = %d  dim2 = %d  dim3 = %d  dim4 = %d  dim3_original = %d  dim4_original = %d  offset = %d  padding_offset = %d  vecSize = %d  laneNum = %d \n" , dim1,  dim2,  dim3,  dim4,  dim3_original,  dim4_original,  offset,  padding_offset,  vecSize,  laneNum);
...
void reorderThresholds(MACTYPE *thresholds, MACTYPE *thresholds_out, int layer){
    ...
    if (layer < LAYER_NUM-1){ //Last layer doesnt need thresholds
        for(unsigned t = 0; t<THRESHOLDS_NUM; t++){
            thresholds_out[t] = thresholds[layer*THRESHOLDS_NUM+t];
            printf(" %d ", thresholds_out[t]);
        ...
        for(unsigned i = 0; i<dim2; i++){
            for(unsigned j = 0; j<dim1; j++){
                for(unsigned vv = 0; vv<VEC_SIZE; vv++){
                    ...
                    // printf("%d  ", output_one_item[k*dim2*dim1*VEC_SIZE + i*dim1*VEC_SIZE + j*VEC_SIZE + vv]);
                }
            }
        }
    }
        ...
        for(unsigned j = 0; j<dim1; j++){
            for(unsigned k = 0; k<(dim3/VEC_SIZE); k++){
                for(unsigned vv = 0; vv<VEC_SIZE; vv++){
                    ...
                    // printf("%d  ", output[k*dim2*dim1*VEC_SIZE + i*dim1*VEC_SIZE + j*VEC_SIZE + vv]);
                }
            }
        }
    }
...
#else
        img1 = img;
        Mat mean_mat = Mat(layer_config_original[0][data_w],layer_config_original[0][data_h], CV_32FC3, Scalar(127,127,127));
#endif
...
void strcopy(OUTDTYPE *str1,OUTDTYPE *str2)
...
void cleanup()
{
    ...
    for(unsigned i = 0; i < num_devices; ++i) {
        ...
        if(fc_2_buf_mac_type && fc_2_buf_mac_type[i]) {
            clReleaseMemObject(fc_2_buf_mac_type[i]);
        }
    }
